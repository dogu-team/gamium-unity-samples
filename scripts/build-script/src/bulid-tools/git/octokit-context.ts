...
export class OctokitContext {
  ...
  async downloadReleaseAsset(owner: string, repo: string, assetId: number, destPath: string): Promise<void> {
    const asset = await this.client.rest.repos.getReleaseAsset({ owner, repo, asset_id: assetId });
    await download(asset.data.url, destPath, { Authorization: `token ${this.token}`, Accept: 'application/octet-stream' });
  }
}
